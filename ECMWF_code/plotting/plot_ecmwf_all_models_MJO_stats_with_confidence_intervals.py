...
datadir = "/perm/mo/more/picsea/ecmwf_track_and_intensity_errors/MJO_phase/"
	...
	for MJO_pairs in [standard_MJO_pairs, alt_MJO_pairs]: #standard_MJO_pairs, alt_MJO_pairs
		...
		for MJO, c in zip(MJO_pairs, colours): 
			...
			if stat_type == "sample_size":
				...
				data = np.genfromtxt(datadir+"2012_2020_MJO_phase"+str(MJO)+"_number_of_forecasts_included_vs_"+obs_track+"_"+fcst_type+".txt")
				...
				data = np.genfromtxt(datadir+"2012_2020_MJO_phase"+str(MJO)+"_average_"+stat_type+"_per_lead_time_vs_"+obs_track+"_"+fcst_type+".with_confidence_intervals_95.txt")
		...
		if MJO_pairs == standard_MJO_pairs:
			plt.savefig("ecmwf_"+fcst_type+".2012-2020.standard_MJO_pairs."+stat_type+".vs_"+obs_track+".with_conf_ints.png", dpi=400, bbox_inches='tight')
		elif MJO_pairs == alt_MJO_pairs:
			plt.savefig("ecmwf_"+fcst_type+".2012-2020.alt_MJO_pairs."+stat_type+".vs_"+obs_track+".with_conf_ints.png", dpi=400, bbox_inches='tight')
		
		plt.close()

for fcst_type in ["ctrl","det","mean"]: #"hres", "ctrl", "mean", "EPS"
