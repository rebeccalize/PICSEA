...
#all years
#year1s = [2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019] #maybe 2010
#year2s = [2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019, 2020] #maybe 2011
...
savedir = "/perm/mo/more/picsea/ecmwf_track_and_intensity_errors/"
...
for obs_track in ["ibtracs"]:#, ["analysis","ibtracs"]:
	...
	for fcst_type in ["det","mean","ctrl"]: #"det","mean", "ctrl"
		print fcst_type
		...
		for y1,y2 in zip(year1s, year2s):
			...
			if y1 == 2015:
				datadir="/perm/mo/more/TIGGE/Y"+str(y1)+str(y2)+"/reformatted_track_files_per_storm_correct/SIO_storms_res2/" 
			...
			def storm_stats(ibtracs_file, analysis_file, nwp_files_list, use_analysis_or_ibtracs, array_len):
				...
				and adds the statistics to an array containing the stats for all storms, for this forecast type and obs track"""
		...
		#calculate the mean and confidence intervals across all the storms we included (across all the years in the arrays, for each forecast type and obs track)
		...
		np.savetxt(savedir + str(year1s[0])+"-"+str(year2s[-1])+"average_location_error_per_lead_time_vs_" + obs_track + "_"+fcst_type+"_with_confidence_intervals_95.txt",  avg_trerr_all_storms, '%.4f')
		np.savetxt(savedir + str(year1s[0])+"-"+str(year2s[-1])+"average_mslp_bias_per_lead_time_vs_" + obs_track + "_"+fcst_type+"_with_confidence_intervals_95.txt", avg_mslp_bias_all_storms, '%.4f')
		np.savetxt(savedir + str(year1s[0])+"-"+str(year2s[-1])+"average_wind_bias_per_lead_time_vs_" + obs_track + "_"+fcst_type+"_with_confidence_intervals_95.txt", avg_wind_bias_all_storms, '%.4f')
	
		np.savetxt(savedir + str(year1s[0])+"-"+str(year2s[-1])+"number_of_forecasts_included_vs_" + obs_track + "_"+fcst_type+".txt",  all_storms_trerr_wgt[:], '%.4f')
