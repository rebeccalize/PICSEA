...
#res1
#year1s=[2011, 2012, 2013, 2014, 2015]
#year2s=[2012, 2013, 2014, 2015, 2016] #BUT!!! the resolution change was in March 2016, so there are extra "SIO_storms" directories,
####### IMPORTANT: need to add an "if statement" if running this resolution, saying if y1 == 2015, use datadir /perm/mo/more/TIGGE/Yy1y2/SIO_storms_res1

#res2
#year1s = [2015, 2016, 2017, 2018, 2019] #There are a couple of storms from the 2015-2016 season that happened after the resolution change on 8th March 2016
#year2s = [2016, 2017, 2018, 2019, 2020]
####### IMPORTANT: So for this resolution, need to add an if statement, saying if y1 == 2015, use datadir /perm/mo/more/TIGGE/Yy1y2/SIO_storms_res2
...
savedir = "/perm/mo/more/picsea/translation_speed_errors/"
...
all_storms_err_ib, all_storms_bias_ib = (np.zeros(array_len) for i in range(2))
all_storms_err_an, all_storms_bias_an = (np.zeros(array_len) for i in range(2))
...
for obs_track in ["analysis","ibtracs"]:#, ["analysis","ibtracs"]:
	...
	for y1, y2 in zip(year1s, year2s):
		...
		#if y1 == 2015:
			#datadir = "/perm/mo/more/TIGGE/Y"+str(y1)+str(y2)+"/reformatted_track_files_per_storm_correct/SIO_storms_res1/"
		...
		for dir, x in zip(season_dirs,range(len(season_dirs))):
			...
			for e,j in zip(es, range(len(es))):
				...
				def storm_stats(ibtracs_file, analysis_file, nwp_files_list, use_analysis_or_ibtracs, array_len,e):
					...
					global all_storms_err_ib
					global all_storms_bias_ib
				
					global all_storms_err_an
					global all_storms_bias_an
					...
					for ff,a in zip(nwp_files_list, range(NT)):
						...
						if not len(indices_fcst) == 0:
							...
							if obs_track == "ibtracs":	
								if np.all(all_storms_err_ib == 0.):
									for lt in range(array_len):
										all_storms_err_ib[lt] = err[lt]
										all_storms_bias_ib[lt] = bias[lt]
									...
									all_storms_err_ib = np.vstack([all_storms_err_ib, err])
				
									all_storms_bias_ib = np.vstack([all_storms_bias_ib, bias])
									
									
							if obs_track == "analysis":	
								if np.all(all_storms_err_an == 0.):
									for lt in range(array_len):
										all_storms_err_an[lt] = err[lt]
										all_storms_bias_an[lt] = bias[lt]
							
								
								else:
									
									
									all_storms_err_an = np.vstack([all_storms_err_an, err])
				
									all_storms_bias_an = np.vstack([all_storms_bias_an, bias])
...
avg_err_all_storms_ib, avg_bias_all_storms_ib = (np.zeros((array_len,3)) for i in range(2))
avg_err_all_storms_an, avg_bias_all_storms_an = (np.zeros((array_len,3)) for i in range(2))

		
for lt in range(array_len):
	avg_err_all_storms_ib[lt,0], avg_err_all_storms_ib[lt,1], avg_err_all_storms_ib[lt,2] = mean_confidence_interval(all_storms_err_ib[:,lt])
	avg_bias_all_storms_ib[lt,0], avg_bias_all_storms_ib[lt,1], avg_bias_all_storms_ib[lt,2] = mean_confidence_interval(all_storms_bias_ib[:,lt])
	
	avg_err_all_storms_an[lt,0], avg_err_all_storms_an[lt,1], avg_err_all_storms_an[lt,2] = mean_confidence_interval(all_storms_err_an[:,lt])
	avg_bias_all_storms_an[lt,0], avg_bias_all_storms_an[lt,1], avg_bias_all_storms_an[lt,2] = mean_confidence_interval(all_storms_bias_an[:,lt])
...
np.savetxt(savedir + str(year1s[0])+"-"+str(year2s[-1])+".average_translation_speed_error_per_lead_time_vs_ibtracs_EPS_with_confidence_intervals_95.txt",  avg_err_all_storms_ib, '%.4f')
np.savetxt(savedir + str(year1s[0])+"-"+str(year2s[-1])+".average_translation_speed_bias_per_lead_time_vs_ibtracs_EPS_with_confidence_intervals_95.txt", avg_bias_all_storms_ib, '%.4f')

np.savetxt(savedir + str(year1s[0])+"-"+str(year2s[-1])+".average_translation_speed_error_per_lead_time_vs_analysis_EPS_with_confidence_intervals_95.txt",  avg_err_all_storms_an, '%.4f')
np.savetxt(savedir + str(year1s[0])+"-"+str(year2s[-1])+".average_translation_speed_bias_per_lead_time_vs_analysis_EPS_with_confidence_intervals_95.txt", avg_bias_all_storms_an, '%.4f')		
