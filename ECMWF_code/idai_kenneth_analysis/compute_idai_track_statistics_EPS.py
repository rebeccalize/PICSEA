...
for e,j in zip(es, range(len(es))):
	...
	for obs_track in ["analysis", "ibtracs"]:#, ["analysis","ibtracs"]:
		...
		def storm_stats(ibtracs_file, analysis_file, nwp_files_list, use_analysis_or_ibtracs, array_len,e):
			...
			np.savetxt(savedir + "idai_EPS"+en+"_each_forecast_location_error_per_lead_time_vs_"+use_analysis_or_ibtracs+".txt", storm_trerr[:,:], '%.4f')
			np.savetxt(savedir + "idai_EPS"+en+"_each_forecast_mslp_bias_per_lead_time_vs_"+use_analysis_or_ibtracs+".txt", storm_mslp_bias[:,:], '%.4f')
			np.savetxt(savedir + "idai_EPS"+en+"_each_forecast_wind_bias_per_lead_time_vs_" + use_analysis_or_ibtracs + ".txt", storm_wind_bias[:, :], '%.4f')
		...
		pattern="*_idai_ECMWF_eps_3way_matched_reformatted.txt"
...
np.savetxt(savedir + "idai_EPS_average_location_error_per_lead_time_vs_ibtracs.txt", eps_trerr_mean_ib[:], '%.4f')
np.savetxt(savedir + "idai_EPS_average_mslp_bias_per_lead_time_vs_ibtracs.txt", eps_mslp_bias_mean_ib[:], '%.4f')
np.savetxt(savedir + "idai_EPS_average_wind_bias_per_lead_time_vs_ibtracs.txt", eps_wind_bias_mean_ib[:], '%.4f')	

np.savetxt(savedir + "idai_EPS_average_location_error_per_lead_time_vs_analysis.txt", eps_trerr_mean_an[:], '%.4f')
np.savetxt(savedir + "idai_EPS_average_mslp_bias_per_lead_time_vs_analysis.txt", eps_mslp_bias_mean_an[:], '%.4f')
np.savetxt(savedir + "idai_EPS_average_wind_bias_per_lead_time_vs_analysis.txt", eps_wind_bias_mean_an[:], '%.4f')
