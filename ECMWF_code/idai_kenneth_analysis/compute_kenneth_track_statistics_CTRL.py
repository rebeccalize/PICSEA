...
for obs_track in ["analysis", "ibtracs"]:#, ["analysis","ibtracs"]:
	...
	def storm_stats(ibtracs_file, analysis_file, nwp_files_list, use_analysis_or_ibtracs, array_len):
		...
		for ff,a in zip(nwp_files_list, range(NT)):
			...
			fcst_lon=fcst_data[np.where(fcst_data[:,0] == 0003),7][0]
			fcst_lat=fcst_data[np.where(fcst_data[:,0] == 0003),8][0]
			fcst_mslp=fcst_data[np.where(fcst_data[:,0] == 0003),9][0]
			fcst_wind=fcst_data[np.where(fcst_data[:,0] == 0003),10][0]
			fcst_datelist = pl.get_dates(fcst_data[np.where(fcst_data[:,0] == 0003),:][0])
		...
		np.savetxt(savedir + "kenneth_CTRL_each_forecast_location_error_per_lead_time_vs_"+use_analysis_or_ibtracs+".txt", storm_trerr[:,:], '%.4f')
		np.savetxt(savedir + "kenneth_CTRL_each_forecast_mslp_bias_per_lead_time_vs_"+use_analysis_or_ibtracs+".txt", storm_mslp_bias[:,:], '%.4f')
		np.savetxt(savedir + "kenneth_CTRL_each_forecast_wind_bias_per_lead_time_vs_" + use_analysis_or_ibtracs + ".txt", storm_wind_bias[:, :], '%.4f')

		np.savetxt(savedir + "kenneth_CTRL_average_location_error_per_lead_time_vs_"+use_analysis_or_ibtracs+".txt", storm_trerr_mean[:], '%.4f')
		np.savetxt(savedir + "kenneth_CTRL_average_mslp_bias_per_lead_time_vs_"+use_analysis_or_ibtracs+".txt", storm_mslp_bias_mean[:], '%.4f')
		np.savetxt(savedir + "kenneth_CTRL_average_wind_bias_per_lead_time_vs_" + use_analysis_or_ibtracs + ".txt", storm_wind_bias_mean[:], '%.4f')
