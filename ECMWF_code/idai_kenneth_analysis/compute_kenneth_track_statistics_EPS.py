...
for e,j in zip(es, range(len(es))):
	...
	for obs_track in ["analysis", "ibtracs"]:#, ["analysis","ibtracs"]:
		...
		def storm_stats(ibtracs_file, analysis_file, nwp_files_list, use_analysis_or_ibtracs, array_len,e):
			...
			np.savetxt(savedir + "kenneth_EPS"+en+"_each_forecast_location_error_per_lead_time_vs_"+use_analysis_or_ibtracs+".txt", storm_trerr[:,:], '%.4f')
			np.savetxt(savedir + "kenneth_EPS"+en+"_each_forecast_mslp_bias_per_lead_time_vs_"+use_analysis_or_ibtracs+".txt", storm_mslp_bias[:,:], '%.4f')
			np.savetxt(savedir + "kenneth_EPS"+en+"_each_forecast_wind_bias_per_lead_time_vs_" + use_analysis_or_ibtracs + ".txt", storm_wind_bias[:, :], '%.4f')

			#np.savetxt(savedir + "idai_EPS"+e+"_average_location_error_per_lead_time_vs_"+use_analysis_or_ibtracs+".txt", storm_trerr_mean[:], '%.4f')
			#np.savetxt(savedir + "idai_EPS"+e+"_average_mslp_bias_per_lead_time_vs_"+use_analysis_or_ibtracs+".txt", storm_mslp_bias_mean[:], '%.4f')
			#np.savetxt(savedir + "idai_EPS"+e+"_average_wind_bias_per_lead_time_vs_" + use_analysis_or_ibtracs + ".txt", storm_wind_bias_mean[:], '%.4f')
		...
		pattern="*_kenneth_ECMWF_eps_3way_matched_reformatted.txt"
		pattern2="kenneth_analysis_*.txt"
		pattern3="kenneth_ibtracs_*.txt"
		...
		for entry in list_of_all_files:
			if fnmatch.fnmatch(entry,pattern):
				...
				analysis_file = datadir+"kenneth_analysis_track_reformatted.txt"
				#print analysis_file
			elif fnmatch.fnmatch(entry,pattern3):
				ibtracs_file = datadir+"kenneth_ibtracs_reformatted.txt"
...
np.savetxt(savedir + "kenneth_EPS_average_location_error_per_lead_time_vs_ibtracs.txt", eps_trerr_mean_ib[:], '%.4f')
np.savetxt(savedir + "kenneth_EPS_average_mslp_bias_per_lead_time_vs_ibtracs.txt", eps_mslp_bias_mean_ib[:], '%.4f')
np.savetxt(savedir + "kenneth_EPS_average_wind_bias_per_lead_time_vs_ibtracs.txt", eps_wind_bias_mean_ib[:], '%.4f')	

np.savetxt(savedir + "kenneth_EPS_average_location_error_per_lead_time_vs_analysis.txt", eps_trerr_mean_an[:], '%.4f')
np.savetxt(savedir + "kenneth_EPS_average_mslp_bias_per_lead_time_vs_analysis.txt", eps_mslp_bias_mean_an[:], '%.4f')
np.savetxt(savedir + "kenneth_EPS_average_wind_bias_per_lead_time_vs_analysis.txt", eps_wind_bias_mean_an[:], '%.4f')
