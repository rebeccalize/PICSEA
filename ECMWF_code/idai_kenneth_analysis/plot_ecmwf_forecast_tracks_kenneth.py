...
def map_nwp_tracks_per_storm(date, outfile):
	...
	m.drawcoastlines(linewidth=0.6, color='k') #darkgray
	m.drawcountries(linewidth=0.6, color='k') #darkgray
	m.fillcontinents(color='silver',alpha=0.25)
	
	hurricane=get_hurricane_symbol()
	
	eps_data = np.genfromtxt(datadir+date+"_kenneth_ECMWF_eps_3way_matched_reformatted.txt", dtype=float, skip_header=1, usecols=np.arange(0,11))
	...
	for tid in e:
		...
		if tid == 1:
			...
			#pp=np.linspace(1,32,8)
			#pp=range(1,32,4)
			#pp=[0,4,8,10,12,16,20,24,28]
			pp=range(no_points)
			colors2=cmap(np.linspace(0,1,no_points))
			print pp			
			for p, cl in zip(pp, colors2):
				...
				if p < 12:
					xx,yy = m(xs[p], ys[p])
					#m.scatter(xx,yy,marker='o', edgecolors=cl,facecolors=cl, s=4,linewidth=0.5,zorder=z)
				elif 12 < p < 20:
					xx,yy = m(xs[p], ys[p])
			...
			m.plot(x,y,linewidth=0.5, color=c, linestyle=ls, latlon=True)
			#if tid == 1.:
				#xx,yy=m(xs[0],ys[0])
				#m.scatter(xx,yy,marker=hurricane, edgecolors=c, facecolors='None',s=25,linewidth=0.6,zorder=10)
	...
	mean_file = datadir+date+"_kenneth_ECMWF_mean_3way_matched_reformatted.txt"
	if os.path.isfile(mean_file):	
		...
		m.plot(x,y, linewidth=0.75, color='mediumblue', latlon=True, linestyle='-')
	...
	det_file = datadir+date+"_kenneth_ECMWF_det_3way_matched_reformatted.txt"
	if os.path.isfile(det_file):
		...
		x,y = m.shiftdata(xs,ys)
		m.plot(x,y, linewidth=0.75, color='#F71735', latlon=True, linestyle='-')
		#if len(xs)>0:
			#xx,yy=m(xs[0],ys[0])
	...
	title="Cyclone Kenneth ECMWF Forecast\n"+str(date)
	...
	mean = plt.Line2D((0, 1), (0, 0), color='mediumblue',linewidth=0.5)
...
map_nwp_tracks_per_storm("2019042312", "kenneth_analysis_track_23April_noctrl.png")

#for d in range(16,29): #last forecast date with Idai was 21st March
	#day = str(d).zfill(2)
	...
	#for h in [0,12]:
		...
		#outfile = "kenneth_ecmwf_forecast_tracks_"+date+".png"
