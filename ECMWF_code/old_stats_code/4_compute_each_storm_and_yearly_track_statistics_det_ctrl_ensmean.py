...
for obs_track in ["analysis", "ibtracs"]:#, ["analysis","ibtracs"]:
	...
	for fcst_type in ["det", "mean", "ctrl"]:
		...
		#np.savetxt(savedir + "average_location_error_per_lead_time_vs_" + obs_track + "_"+fcst_type+".txt",  avg_trerr_all_storms[:], '%.4f')
		np.savetxt(savedir + "average_mslp_bias_per_lead_time_vs_" + obs_track + "_"+fcst_type+".txt", avg_mslp_bias_all_storms[:], '%.4f')
		np.savetxt(savedir + "average_wind_bias_per_lead_time_vs_" + obs_track + "_"+fcst_type+".txt", avg_wind_bias_all_storms[:], '%.4f')		
