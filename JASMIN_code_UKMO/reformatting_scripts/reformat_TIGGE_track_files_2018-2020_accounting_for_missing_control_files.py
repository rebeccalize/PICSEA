import sys
sys.path.insert(1, '/home/users/emerton/analysis_scripts') #allows to import picsea_library from a different directory, 
...
datadir = "/gws/nopw/j04/klingaman/emerton/ukmo_TIGGE_analysis/reformatted_track_files_per_storm/Y{}{}/tr{}/"

savedir = "/gws/nopw/j04/klingaman/emerton/ukmo_TIGGE_analysis/reformatted_track_files_per_storm/Y{}{}/tr{}/"
...
def rewrite_track_file_ensemble_with_control(indir, infiles, outfile):
	"""Takes several input files containing track information from TRACK and rewrites
	them in a more convenient format.
...
def rewrite_track_file_ensemble_WITHOUT_control(indir, infiles, outfile):
	"""Takes several input files containing track information from TRACK and rewrites
	them in a more convenient format.
	...
	for infile in infiles:
		...
		for line in rl:
			...
			if track_id > 1: #(don't want to include the analysis or ibtracs (0 and 1), but this time no control so ensemble start at 2
			#but no way to guarantee the control isn't in there using find_ids, as some ensembles
			# have no TC in the foreacst..., so numbers not usually/always consecutive from 3 to 52...)
...
def run_rewrite_track_files(date, track, which_track):
	...
	if which_track == "analysis":
	...
	elif which_track == "ensemble_with_control":
	    find_id = 3
	    ...
	    rewrite_track_file_ensemble_with_control(indir, infiles, outfile)
	    
	elif which_track == "ensemble_without_control":
	    find_id = 2
	    outfile = os.path.join(savedir.format(y1,y2,track), ENS_REFORMAT_FILENAME.format(date,track))
	    ...
	    rewrite_track_file_ensemble_WITHOUT_control(indir, infiles, outfile)
...
#AND then as a bonus, in 2018 and 2019, we ran the tracking and matching without the control forecast, as there were *so* many dates it was missing in the archive
#BUT the control is saved within the ensemble file, so for dates with the control missing, the track ids (i) are different to those with the control - 
#we don't want to incorrectly assign a control forecast, so we need to treat these differently and search through the list of missing dates 


#get the dates of all the forecasts for this storm
dates_with_control=[]
dates_without_control=[]

missing_control_dates = np.genfromtxt("/home/users/emerton/analysis_scripts/reformatting_scripts/UKMO_TIGGE_missing_control_dates_2018-2020.txt", dtype=str)
print "MISSING CONTROL DATES"
print missing_control_dates
...
for filename in os.listdir(search_dir):
	...
	if date in missing_control_dates:
		dates_without_control.append(date)
	else:
	
   		dates_with_control.append(date)
		
		
print 'dates with control:'
print dates_with_control

print 'dates without control:'
print dates_without_control

#loop over all the dates and reformat the track files
for d,i in zip(dates_with_control,range(len(dates_with_control))):
	...
	if i == 0:
		...
		run_rewrite_track_files(d,track,"ensemble_with_control")
...
for d,i in zip(dates_without_control,range(len(dates_without_control))):
	...
	if i == 0:
		...
		run_rewrite_track_files(d,track,"ensemble_without_control")
