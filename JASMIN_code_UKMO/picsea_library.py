...
def exclude_days(lead=None):
	...
	strs = np.genfromtxt(os.path.join(UKMO_DIR, 'MISSING_PICSEA.txt'), comments='#', delimiter=13, usecols=0, dtype=str)
...
def interpolate_track_file(infile, outfile):
	...
	print 'interpolating track file', infile
	...
	for itrack in xrange(1, ntrack + 1):
		...
		if len(ain_t) > 0:
			...
			y_interp, m_interp, d_interp, h_interp = [dt_inv_func(datetimes_interp, att).reshape((npoint - 1, 1)) for att in atts]

			# Create array for all interpolated data for this track
			ain_t_interp = np.concatenate((ain_t_interp0[:, :3], y_interp, m_interp, d_interp, h_interp, ain_t_interp0[:, 3:]), axis=1)
		...
		#raise ValueError('Number of rows of output data is %d; should be %d' % (count_rows, nrows - ntrack))
...
def map_nwp_tracks_per_storm(nwp_infile_list, analysis_file,ibtracs_file, outfile, region,cat,name):
	...
	for file_in_list,c in zip(nwp_infile_list,colors):
		...
		# load in data from a specified TRACK file (reformatted & interpolated)
		data = np.genfromtxt(file_in_list, dtype=float, skip_header=1,usecols=np.arange(0,11))
...
def map_composite_data(data, lats, lons, outfile, region, cmap, label, bounds,contour_or_grid,dtype=None):
	...
	if dtype == 'bias':
		...
		im.cmap.set_bad('lightgrey')  # set missing values to white/transparent
...
def track_density(infile, outfile, _source, year, month, box_size=2.5,
	...
	# JRA55
	if _source == 'ibtracs' or _source == 'analysis':


		# Get track data, and select specified year and month
		ain = np.genfromtxt(infile, dtype=float, skip_header=1, usecols=range(3, 9))
		aint = ain[np.where((ain[:, 0] == year) & (ain[:, 1] == month))]

		# Count days
		t1 = datetime.datetime(year, month, 1)
		t2 = datetime.datetime(year + 1 if month == 12 else year, 1 if month == 12 else month + 1, 1)
