...
for dir, x in zip(season_dirs,range(len(season_dirs))):
	...
	eps_avg_trerr_ib, eps_avg_mslp_bias_ib, eps_avg_wind_bias_ib = (np.zeros((len(es),array_len)) for i in range(3))
	eps_avg_trerr_an, eps_avg_mslp_bias_an, eps_avg_wind_bias_an = (np.zeros((len(es),array_len)) for i in range(3))
				...
				for ff,a in zip(nwp_files_list, range(NT)):
					...
					if not len(indices_fcst) == 0:
						...
						if obs_track == "ibtracs":
							...
							#if not len(all_storms_trerr_ib) > 1:
								for lt in range(array_len):
							...
							print "len(all_storms_trerr_an)"
							print len(all_storms_trerr_an)
						
							#print all_storms_trerr_an
							if np.all(all_storms_trerr_an == 0.):
							#if not len(all_storms_trerr_an) > 1:
								for lt in range(array_len):
...
#then calculate the mean (and confidence intervals) across all storms in this year, of the average error across all the ensemble members... etc etc	
...
np.savetxt(savedir + "average_location_error_per_lead_time_vs_ibtracs_EPS_with_confidence_intervals_95.txt",  avg_trerr_all_storms_ib, '%.4f')
np.savetxt(savedir + "average_mslp_bias_per_lead_time_vs_ibtracs_EPS_with_confidence_intervals_95.txt", avg_mslp_bias_all_storms_ib, '%.4f')
np.savetxt(savedir + "average_wind_bias_per_lead_time_vs_ibtracs_EPS_with_confidence_intervals_95.txt", avg_wind_bias_all_storms_ib, '%.4f')

np.savetxt(savedir + "average_location_error_per_lead_time_vs_analysis_EPS_with_confidence_intervals_95.txt",  avg_trerr_all_storms_an, '%.4f')
np.savetxt(savedir + "average_mslp_bias_per_lead_time_vs_analysis_EPS_with_confidence_intervals_95.txt", avg_mslp_bias_all_storms_an, '%.4f')
np.savetxt(savedir + "average_wind_bias_per_lead_time_vs_analysis_EPS_with_confidence_intervals_95.txt", avg_wind_bias_all_storms_an, '%.4f')		
		
np.savetxt(savedir + "number_of_forecasts_included_vs_ibtracs_EPS.txt",  all_storms_sample_size_ib, '%.4f')
np.savetxt(savedir + "number_of_forecasts_included_vs_analysis_EPS.txt",  all_storms_sample_size_an, '%.4f')
