...
savedir = "/gws/nopw/j04/klingaman/emerton/ukmo_nwp_analysis/translation_speed_errors/"
...
for obs_track in ["analysis", "ibtracs"]:#, ["analysis","ibtracs"]:
	...
	print all_storms_speed_err
	print all_storms_speed_bias
	...
	np.savetxt(savedir + str(year1s[0])+"-"+str(year2s[-1])+".average_translation_speed_error_per_lead_time.vs_" + obs_track + ".with_confidence_intervals.txt",  avg_err_all_storms, '%.4f')
	np.savetxt(savedir + str(year1s[0])+"-"+str(year2s[-1])+".average_translation_speed_bias_per_lead_time.vs_" + obs_track + ".with_confidence_intervals.txt", avg_bias_all_storms, '%.4f')
	
	np.savetxt(savedir + str(year1s[0])+"-"+str(year2s[-1])+".number_of_forecasts_included.vs_" + obs_track + ".txt",  all_storms_wgt, '%.4f')	
