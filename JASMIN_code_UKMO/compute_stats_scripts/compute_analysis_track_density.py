...
REFORMAT_DIR = "/gws/nopw/j04/klingaman/emerton/analysis_reformatted_track_files_yearmonth/"

INTERPOLATE_FILENAME = 'analysis.{0}{1}interpolated.txt'
#INTERPOLATE_FILENAME = 'analysis.%Y%m%d%H.tcident.interp'

 
DENSITY_DIR = "/gws/nopw/j04/klingaman/emerton/analysis_track_density/"

DENSITY_FILENAME = "analysis.density.{0}{1}.nc"
...
def track_density_year_month(y, year1, year2, month, box_size=2.5, threeway=True):
	...
	pl.track_density(infile, outfile, 'analysis',year=int(y),month=int(month), box_size=2.5, exclude=exclude, contributing_days=True)
...
def track_density_year(year1, year2, box_size=2.5, threeway=True):
	...
	outfile = "analysis.density.{0}{1}.nc".format(year1,year2)
