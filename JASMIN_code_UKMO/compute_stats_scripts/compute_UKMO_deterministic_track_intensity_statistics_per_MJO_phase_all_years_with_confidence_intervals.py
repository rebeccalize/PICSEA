...
for obs_track in ["ibtracs"]:#, ["analysis","ibtracs"]:
	...
	print "all storms trerr weight:"
	print all_storms_trerr_wgt
	
	
	new_version_wgt = np.zeros(array_len)
	new_version_sum = np.zeros(array_len)
	
	for lt in range(array_len):
	
		new_version_wgt[lt] = len([0 for x in this_mjo_trerr[:,lt] if not math.isnan(x)])
		new_version_sum[lt] = np.nansum(this_mjo_trerr[:,lt])
		
	print "new version trerr weight:"
	print new_version_wgt
	print "new version trerr sum:"
	print new_version_sum
	...
	print "new version trerr mean:"
	print avg_trerr_all_storms[:,0]

	np.savetxt(savedir + str(year1s[0])+"_"+str(year2s[-1])+"_MJO_phase"+str(MJO1)+str(MJO2)+"_average_location_error_per_lead_time_vs_"+obs_track+".with_confidence_intervals.txt", avg_trerr_all_storms, '%.4f')
	np.savetxt(savedir + str(year1s[0])+"_"+str(year2s[-1])+"_MJO_phase"+str(MJO1)+str(MJO2)+"_average_mslp_bias_per_lead_time_vs_"+obs_track+".with_confidence_intervals.txt", avg_mslp_bias_all_storms, '%.4f')
	np.savetxt(savedir + str(year1s[0]) + "_" + str(year2s[-1]) + "_MJO_phase"+str(MJO1)+str(MJO2)+"_average_wind_bias_per_lead_time_vs_" + obs_track + ".with_confidence_intervals.txt", avg_wind_bias_all_storms, '%.4f')
	
	np.savetxt(savedir + str(year1s[0])+"_"+str(year2s[-1])+"_MJO_phase"+str(MJO1)+str(MJO2)+"_number_of_forecasts_included_vs_" + obs_track + ".txt",  all_storms_trerr_wgt, '%.4f')
