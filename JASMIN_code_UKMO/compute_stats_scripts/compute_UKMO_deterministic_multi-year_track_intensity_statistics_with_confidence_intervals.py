...
savedir = "/gws/nopw/j04/klingaman/emerton/UKMO_HRES_RESULTS/track_and_intensity_errors/"
...
def mean_confidence_interval(data, mean_or_median, confidence=0.95):
    a = 1.0 * np.array(data)
    n = len(a)
    if mean_or_median == 'mean':
    	m, se = np.nanmean(a), scipy.stats.sem(a, nan_policy='omit')
    elif mean_or_median == 'median':
    	m, se = np.nanmedian(a), scipy.stats.sem(a, nan_policy='omit')
...
for obs_track in ["ibtracs","analysis"]:#, ["analysis","ibtracs"]:
	print obs_track
	...
	for y1, y2 in zip(year1s,year2s):
		...
		datadir="/gws/nopw/j04/klingaman/emerton/reformatted_sh_track_output/UKMO/HRES/reformatted_track_files_by_storm/"+str(y1)+"_"+str(y2)+"/SIO_storms/"
	...
	for lt in range(array_len):
		...
		avg_trerr_all_storms[lt,0], avg_trerr_all_storms[lt,1], avg_trerr_all_storms[lt,2] = mean_confidence_interval(this_category_trerr[:,lt], 'mean')
		avg_mslp_bias_all_storms[lt,0], avg_mslp_bias_all_storms[lt,1], avg_mslp_bias_all_storms[lt,2] = mean_confidence_interval(this_category_mslp_bias[:,lt], 'mean')
		avg_wind_bias_all_storms[lt,0], avg_wind_bias_all_storms[lt,1], avg_wind_bias_all_storms[lt,2] = mean_confidence_interval(this_category_wind_bias[:,lt], 'mean')
	...
	np.savetxt(savedir + str(year1s[0])+"_"+str(year2s[-1])+".ukmo_hres.average_location_error_per_lead_time_vs_"+obs_track+"_with_confidence_intervals_95.txt", avg_trerr_all_storms, '%.4f')
	np.savetxt(savedir + str(year1s[0])+"_"+str(year2s[-1])+".ukmo_hres.average_mslp_bias_per_lead_time_vs_"+obs_track+"_with_confidence_intervals_95.txt", avg_mslp_bias_all_storms, '%.4f')
	np.savetxt(savedir + str(year1s[0])+"_"+str(year2s[-1])+".ukmo_hres.average_wind_bias_per_lead_time_vs_" + obs_track + "_with_confidence_intervals_95.txt", avg_wind_bias_all_storms, '%.4f')


	
	np.savetxt(savedir + str(year1s[0])+"_"+str(year2s[-1])+".ukmo_hres.number_of_forecasts_included_vs_" + obs_track + ".txt",  all_storms_trerr_wgt[:], '%.4f')
