...
savedir = "/gws/nopw/j04/klingaman/emerton/ukmo_nwp_analysis/track_and_intensity_errors/"+str(y1)+"_"+str(y2)+"/SIO_storms/"
...
for obs_track in ["analysis", "ibtracs"]:#, ["analysis","ibtracs"]:
	...
	def storm_stats(ibtracs_file, analysis_file, nwp_files_list, use_analysis_or_ibtracs, array_len):
		...
		np.savetxt(savedir + dir + "_each_forecast_location_error_per_lead_time_vs_"+use_analysis_or_ibtracs+".txt", storm_trerr[:,:], '%.4f')
		np.savetxt(savedir + dir + "_each_forecast_mslp_bias_per_lead_time_vs_"+use_analysis_or_ibtracs+".txt", storm_mslp_bias[:,:], '%.4f')
		np.savetxt(savedir + dir + "_each_forecast_wind_bias_per_lead_time_vs_" + use_analysis_or_ibtracs + ".txt", storm_wind_bias[:, :], '%.4f')

		np.savetxt(savedir + dir + "_average_location_error_per_lead_time_vs_"+use_analysis_or_ibtracs+".txt", storm_trerr_mean[:], '%.4f')
		np.savetxt(savedir + dir + "_average_mslp_bias_per_lead_time_vs_"+use_analysis_or_ibtracs+".txt", storm_mslp_bias_mean[:], '%.4f')
		np.savetxt(savedir + dir + "_average_wind_bias_per_lead_time_vs_" + use_analysis_or_ibtracs + ".txt", storm_wind_bias_mean[:], '%.4f')
	...
	#np.savetxt(savedir + str(y1)+"_"+str(y2)+"_average_location_error_per_lead_time_vs_"+obs_track+".txt", avg_trerr_all_storms[:], '%.4f')
	#np.savetxt(savedir + str(y1)+"_"+str(y2)+"_average_mslp_bias_per_lead_time_vs_"+obs_track+".txt", avg_mslp_bias_all_storms[:], '%.4f')
	#np.savetxt(savedir + str(y1) + "_" + str(y2) + "_average_wind_bias_per_lead_time_vs_" + obs_track + ".txt", avg_wind_bias_all_storms[:], '%.4f')

	np.savetxt(savedir + "average_location_error_per_lead_time_vs_" + obs_track + ".txt",  avg_trerr_all_storms[:], '%.4f')
	np.savetxt(savedir + "average_mslp_bias_per_lead_time_vs_" + obs_track + ".txt", avg_mslp_bias_all_storms[:], '%.4f')
	np.savetxt(savedir + "average_wind_bias_per_lead_time_vs_" + obs_track + ".txt", avg_wind_bias_all_storms[:], '%.4f')
	
	#np.savetxt(savedir + str(y1)+"_"+str(y2)+"_number_of_forecasts_included_vs_" + obs_track + ".txt",  all_storms_trerr_wgt[:], '%.4f')
	
	np.savetxt(savedir + "number_of_forecasts_included_vs_" + obs_track + ".txt",  all_storms_trerr_wgt[:], '%.4f')
	#np.savetxt(savedir + "average_mslp_bias_per_lead_time_vs_" + obs_track + ".txt", avg_mslp_bias_all_storms[:], '%.4f')
	#np.savetxt(savedir + "average_wind_bias_per_lead_time_vs_" + obs_track + ".txt", avg_wind_bias_all_storms[:], '%.4f')
