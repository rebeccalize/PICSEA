...
array_len=30 #number of forecast timesteps (7 days x 4 forecasts + lead time 0 /analysis)
...
savedir = "/gws/nopw/j04/klingaman/emerton/ukmo_nwp_analysis/track_and_intensity_errors/"+str(y1)+"_"+str(y2)+"/SIO_storms/v2/"

if not os.path.exists(savedir):
    os.makedirs(savedir)

#Find out the number of storms in this TC season and create array of the IDs so we can loop over them
season_dirs_N320 = ['tr0001', 'tr0002','tr0003','tr0006','tr0009','tr0013','tr0015','tr0018','tr0021']
season_dirs_N512 = ['tr0024', 'tr0026','tr0028']

for resolution in ['N320', 'N512']:

	if resolution == 'N320':
		season_dirs=season_dirs_N320
	elif resolution == 'N512':
		season_dirs = season_dirs_N512
	...
	for obs_track in ["analysis", "ibtracs"]:#, ["analysis","ibtracs"]:
		...
		np.savetxt(savedir + "average_location_error_per_lead_time_vs_"+obs_track+"_"+resolution+".txt", avg_trerr_all_storms[:], '%.4f')
		np.savetxt(savedir + "average_mslp_bias_per_lead_time_vs_"+obs_track+"_"+resolution+".txt", avg_mslp_bias_all_storms[:], '%.4f')
		np.savetxt(savedir + "average_wind_bias_per_lead_time_vs_" + obs_track + "_"+resolution+".txt", avg_wind_bias_all_storms[:], '%.4f')

		np.savetxt(savedir + "number_of_forecasts_included_vs_" + obs_track + "_" + resolution + ".txt",  all_storms_trerr_wgt[:], '%.4f')
