...
datadir="/gws/nopw/j04/klingaman/emerton/ukmo_nwp_analysis/IDAI/reformatted_idai_forecast_tracks/"
...
for obs_track in ["analysis", "ibtracs"]:#, ["analysis","ibtracs"]:
	...
	def storm_stats(ibtracs_file, analysis_file, nwp_files_list, use_analysis_or_ibtracs, array_len):
		...
		np.savetxt(savedir + "idai_each_forecast_location_error_per_lead_time_vs_"+use_analysis_or_ibtracs+".txt", storm_trerr[:,:], '%.4f')
		np.savetxt(savedir + "idai_each_forecast_mslp_bias_per_lead_time_vs_"+use_analysis_or_ibtracs+".txt", storm_mslp_bias[:,:], '%.4f')
		np.savetxt(savedir + "idai_each_forecast_wind_bias_per_lead_time_vs_" + use_analysis_or_ibtracs + ".txt", storm_wind_bias[:, :], '%.4f')

		np.savetxt(savedir + "idai_average_location_error_per_lead_time_vs_"+use_analysis_or_ibtracs+".txt", storm_trerr_mean[:], '%.4f')
		np.savetxt(savedir + "idai_average_mslp_bias_per_lead_time_vs_"+use_analysis_or_ibtracs+".txt", storm_mslp_bias_mean[:], '%.4f')
		np.savetxt(savedir + "idai_average_wind_bias_per_lead_time_vs_" + use_analysis_or_ibtracs + ".txt", storm_wind_bias_mean[:], '%.4f')
	...
	pattern="idai_UKMO*_reformatted.txt"
