...
fcstdatadir = "/gws/nopw/j04/klingaman/emerton/ecmwf/"
...
def map_nwp_tracks_per_storm(nwp_infile_list,ibtracs_file, outfile, region,cat,name,date):
	...
	m.drawcoastlines(linewidth=0.4, color='gray') #darkgray
	m.drawcountries(linewidth=0.4, color='gray') #darkgray
	...
	colors = cm.PuRd(np.linspace(0.3, 1, no_tracks)) # Wistia BuPu
	...
	nwp = plt.Line2D((0, 1), (0, 0), color='crimson',linewidth=0.5)
	legend = ax.legend((ib, nwp), ['Observed Track', 'ECMWF Det. Forecasts'],title=title, fontsize=5, loc='lower left')
...
nwp_infile_list = []
dates = [2019030100,2019030112,2019030200,2019030212, 2019030300, 2019030312,2019030400,2019030412, 2019030500, 2019030512,2019030600,2019030612,2019030700,2019030712,2019030800,2019030812,2019030900,2019030912,2019031000,2019031012,2019031100,2019031112,2019031200,2019031212,2019031300,2019031312,2019031400]
dates2 = [2019030100,2019030200,2019030400, 2019030500,2019030600,2019030700,2019030800,2019030900,2019031000,2019031100,2019031200,2019031300,2019031400]
count=0
for date in dates:
	count+=1
	nwp_infile_list.append(fcstdatadir+"idai_ECMWF_det_"+str(date)+"_reformatted.txt")
	outfile="idai_ecmwf_det_forecast_tracks_"+str(count)+".png"
	print "mapping no. ", count
	map_nwp_tracks_per_storm(nwp_infile_list,ibtracs_file,outfile,"SIO","3","Idai",date)

outfile = "idai_ecmwf_det_forecast_tracks.png"	
