...
#Loads of things say ibtracs, because this script is based on the original script which did these composites using ibtracs
#It's actually been updated to use the analysis files... 


IBTRACS_DIR = "/gws/nopw/j04/klingaman/emerton/analysis_reformatted_track_files_yearmonth/"
IBTRACS_FILE = "analysis.%d%dinterpolated.txt"
COMP_PCP_TC_IBTRACS_FILE = "analysis.trmm.comp_pcp_tc.%d%02d.nc"

COMP_PCP_TC_IBTRACS_DIR = "/gws/nopw/j04/klingaman/emerton/analysis_trmm_tc_precip_composites/"
...
def composite_pcp_tc_ibtracs_year_month(yyyy, month):
    ...
    """Computes a composite of the precipitation due to (actual - from analysis) TCs for a particular
    year and month - using analysis TCs, NOT NWP!
    ...
    elif (yyyy, month) == (2017,7):
    	count_days_res = {'n768': 0, 'n1280': 0}
	start_n1280 = datetime.datetime(2017,7,11)
    while t1 <= t2:
        if t1 not in exclude:
		    ...
		    count_days_res['n768'] += 0.5
		    count_days_res['n1280'] += 0.5
		else:
		    count_days_res['n1280'] += 1
        t1 += dt
    ...
    if (yyyy, month) == (2010, 3):
        ...
        pcp_list_res = {'n768': iris.cube.CubeList(),
                        'n1280': iris.cube.CubeList()}
    ...
    for t_id, y, m, d, h, lon, lat in ain_use:
        ...
        if (yyyy, month) == (2010, 3):
            ...
            if datetime.datetime(y, m, d, h) < start_n1280:
                pcp_list_res['n768'].append(this_pcp)
            else:
                pcp_list_res['n1280'].append(this_pcp)
    ...
    if (yyyy, month) in [(2010, 3), (2014, 7), (2017,7)]:
...
def composite_pcp_tc_year(year1, year2, lead=None):
    ...
    outfile = "analysis.trmm.comp_pcp_tc.%d-%d.nc" % (year1, year2)
    outpath = os.path.join(COMP_PCP_TC_IBTRACS_DIR, outfile)

    pl.add_files(infiles, outpath, cube_count=12)

for year in [2006, 2007, 2008, 2009,2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018]: #2006, 2007, 2008, 2009,2010, 2011, 2012, 2013, 2014, 2015,
    if year == 2006:
