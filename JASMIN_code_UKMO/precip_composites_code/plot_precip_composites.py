...
datadir = "/gws/nopw/j04/klingaman/emerton/ibtracs_precip_composites/"
...
for y1, y2 in zip(year1s, year2s):
	...
	infile = datadir + "ibtracs.comp_pcp_tc."+str(y1)+"-"+str(y2)+".nc"
	print infile
	outfile = "ibtracs.comp_pcp_tc."+str(y1)+"-"+str(y2)+"_mmday.png"
	ff = Dataset(infile, 'r')
...
outfile = "ibtracs.comp_pcp_tc."+str(year1s[0])+"-"+str(year2s[-1])+"_mmday.png"
label = " "#""TC-related precipitation (mm) " + str(year1s[0]) + "-" + str(year2s[-1])
