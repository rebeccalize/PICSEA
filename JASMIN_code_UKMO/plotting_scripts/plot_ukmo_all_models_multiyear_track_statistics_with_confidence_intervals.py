	...
	for y1, y2 in zip(res_start_years, res_end_years):
		...
		if stat_type == "sample_size":
			...
			if fcst_type == "hres":
				...
				data = np.genfromtxt(datadirdet+str(y1)+"_"+str(y2)+".ukmo_hres.number_of_forecasts_included_vs_"+obs_track+".txt")
				
			elif fcst_type == "EPS":
				
				data = np.genfromtxt(datadirens+str(y1)+"-"+str(y2)+".number_of_forecasts_included_vs_"+obs_track+"_"+fcst_type+".txt")
			else:
				data = np.genfromtxt(datadirens+str(y1)+"-"+str(y2)+"number_of_forecasts_included_vs_"+obs_track+"_"+fcst_type+".txt")
			...
			if fcst_type == "hres":
				...
				data = np.genfromtxt(datadirdet+str(y1)+"_"+str(y2)+".ukmo_hres.average_"+stat_type+"_per_lead_time_vs_"+obs_track+"_with_confidence_intervals_95.txt")
