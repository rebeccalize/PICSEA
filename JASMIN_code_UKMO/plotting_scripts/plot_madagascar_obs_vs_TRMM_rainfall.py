...
for year in [2006,2007,2008,2009,2010,2011,2012,2013,2014,2015,2016,2017,2018]: #2006-2018
...
omax = max(max(pcp_dict["obs"].items(), key = lambda x: x[1])[1])
tmax = max(max(pcp_dict["trmm"].items(), key = lambda x: x[1])[1])
...
for ID in station_IDs:
	...
	plt.scatter(pcp_dict["obs"][ID][:], pcp_dict["trmm"][ID][:],s=5,color='navy')	
...
plt.savefig("madagascar_obs_vs_trmm_rainfall_scatter.png", dpi=400,bbox_inches='tight')		
