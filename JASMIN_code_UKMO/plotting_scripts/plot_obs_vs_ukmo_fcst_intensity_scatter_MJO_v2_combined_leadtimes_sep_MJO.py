...
for MJO,c in zip([1,2,3,4,5,6,7,8],['#EF476F', '#F78C6B', '#FFD166', '#06D6A0','#34E4EA','#118AB2','#073B4C','#9B7EDE']):
	...
	if error_type == 'mslp':
			...
			for i in range(len(fcst_wind)):
				...
				if MJO_phase[i] == MJO:
					...
					if fcst_mslp[i] > 10000 or obs_mslp[i] > 10000:
						...
						plt.scatter(obs_mslp[i], fcst_mslp[i],s=ss,color=c,alpha=a)
		...
		legend = ax.legend(handles=[p1,p2,p3,p4,p5,p6,p7,p8], fontsize=9, loc='lower right', title = 'MJO '+str(MJO))
		legend._legend_box.align = "center"
	
	
		plt.savefig("scatter.IBTrACS_vs_UKMO_"+fcst_type+".MSLP.MJO_phase_"+str(MJO)+".coloured_by_leadtime.png", dpi=400,bbox_inches='tight')
			
		print "scatter.IBTrACS_vs_UKMO_"+fcst_type+".MSLP.MJO_phase_"+str(MJO)+".coloured_by_leadtime.png"
			...
			for i in range(len(fcst_wind)):
				...
				if MJO_phase[i] == MJO:
					...
					if fcst_wind[i] > 10000 or obs_wind[i] > 10000:
						...
						plt.scatter(obs_wind[i], fcst_wind[i],s=ss,color=c,alpha=a)
		...
		legend = ax.legend(handles=[p1,p2,p3,p4,p5,p6,p7,p8], fontsize=10, loc='upper left', title = 'MJO '+str(MJO))
		legend._legend_box.align = "left"
	
	
		plt.savefig("scatter.IBTrACS_vs_UKMO_"+fcst_type+".wind_speed.MJO_phase_"+str(MJO)+".coloured_by_leadtime.png", dpi=400,bbox_inches='tight')
			
		print "scatter.IBTrACS_vs_UKMO_"+fcst_type+".wind_speed.MJO_phase_"+str(MJO)+".coloured_by_leadtime.png"
